# as this is an initial testing of SEAL, almost the whole code is from https://github.com/penghao-wu/vstar
...
def get_missing_object_labels_seal(vqa_llm, image, question, missing_objects_msg, annotation=None):
    ...
    return missing_objects, prediction

def get_missing_object_labels_correct(vqa_llm, image, question, missing_objects_msg, annotation):
    return annotation['target_object'], "no prediction needed, the object labels are correct"

def get_bounding_boxes_seal(missing_objects, image_path, vsm, args, annotation):
    search_result = []
...
def get_bounding_boxes_correct(missing_objects, image_path, vsm, args, annotation):
    search_result = []
    for idx, object_name in enumerate(missing_objects):
        search_result.append({'bbox': annotation['bbox'][idx], 'name': object_name})
    return search_result

def get_multiple_choice_seal(vqa_llm, image_path, question, search_result, annotation, missing_objects, focus_msg):
...
def eval_model(args, test_name):
    ...
    for test_type in ['direct_attributes', 'relative_position']:
        ...
        for image_file in tqdm(image_files):
            ...
            if test_name == "initial_seal_testing":
                missing_objects, prediction = get_missing_object_labels_seal(vqa_llm, image, question, missing_objects_msg, annotation)
            else:
                missing_objects, prediction = get_missing_object_labels_correct(vqa_llm, image, question, missing_objects_msg, annotation)

            if test_name == "correct_bounding_boxes":
                search_result = get_bounding_boxes_correct(missing_objects, image_path, vsm, args, annotation)
            else:
                search_result = get_bounding_boxes_seal(missing_objects, image_path, vsm, args, annotation)

            correct, options, option_chosen = get_multiple_choice_seal(vqa_llm, image_path, question, search_result, annotation, missing_objects, focus_msg)
...
if __name__ == "__main__":
    # here you can set the test type
    types = ["initial_seal_testing", "correct_missing_object_labels", "correct_bounding_boxes"]
    ...
    for test_type in types:
        output_path = "eval_results_" + test_type + ".json"
        results = eval_model(args, test_type)
