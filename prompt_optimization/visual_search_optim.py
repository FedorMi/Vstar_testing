# as this is a modified visual search file to allow for prompt testing on V*, almost the whole code is from https://github.com/penghao-wu/vstar
...
class Prioritize:
    ...
    def __lt__(self, other):
        return self.priority < other.priority
def visual_search_queue(vsm, image, target_object_name, current_patch, search_path, queue, prompt, smallest_size=224, confidence_high=0.5, target_cue_threshold=6.0, target_cue_threshold_decay=0.7, target_cue_threshold_minimum=3.0):
    ...
    while(not queue.empty()):
        ...
        success, search_path, all_valid_boxes = visual_search_queue(vsm, image, target_object_name, patch_chosen, search_path, queue, prompt, smallest_size=smallest_size, confidence_high=confidence_high, target_cue_threshold=target_cue_threshold, target_cue_threshold_decay=target_cue_threshold_decay, target_cue_threshold_minimum=target_cue_threshold_minimum)
        if success:
            return success, search_path, all_valid_boxes
    return False, search_path, None


def visual_search(vsm, image, target_object_name, target_bbox, smallest_size, prompt = "Please locate the {} in this image.", confidence_high=0.5, confidence_low=0.3, target_cue_threshold=6.0, target_cue_threshold_decay=0.7, target_cue_threshold_minimum=3.0, visualize=False, save_path=None):
    if visualize:
    ...
    queue = PriorityQueue()
    search_successful, search_path, all_valid_boxes = visual_search_queue(vsm, image, target_object_name, init_patch, search_path, queue, prompt, smallest_size=smallest_size, confidence_high=confidence_high, target_cue_threshold=target_cue_threshold, target_cue_threshold_decay=target_cue_threshold_decay, target_cue_threshold_minimum=target_cue_threshold_minimum)
