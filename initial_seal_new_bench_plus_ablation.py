...
json_files = list(filter(lambda file: '.json' in file, os.listdir("more_initial_seal_jsons")))


for i in json_files:
    path = os.path.join("more_initial_seal_jsons", i)
    ...
    plt.title("Irrelevant missing objects Ablation Results")
    plt.legend()
    save_path = os.path.join("hyperparameter_plots", f"irrelevant_missing_objects_ablation_results.png")
