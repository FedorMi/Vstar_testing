...
def parse_args(args, parameter_name, parameter_value, parameter_value_optional=None):
    confidence_low = 0.3
    confidence_high = 0.5
    ...
    if parameter_name == "confidence":
        confidence_low = parameter_value
        confidence_high = parameter_value_optional
    elif parameter_name == "target_cue_threshold":
        target_cue_threshold = parameter_value
    elif parameter_name == "target_cue_threshold_decay":
        target_cue_threshold_decay = parameter_value
    elif parameter_name == "target_cue_threshold_minimum":
        target_cue_threshold_minimum = parameter_value
    ...
    elif parameter_name == "model_max_length":
        model_max_length = parameter_value

    print(confidence_low)
    ...
    parser.add_argument("--confidence_low", default=confidence_low, type=float)
    parser.add_argument("--confidence_high", default=confidence_high, type=float)
    parser.add_argument("--target_cue_threshold", default=target_cue_threshold, type=float)
    parser.add_argument("--target_cue_threshold_decay", default=target_cue_threshold_decay, type=float)
    parser.add_argument("--target_cue_threshold_minimum", default=target_cue_threshold_minimum, type=float)
    parser.add_argument("--minimum_size_scale", default=minimum_size_scale, type=float)
    parser.add_argument("--minimum_size", default=minimum_size, type=int)
    parser.add_argument("--model_max_length", default=model_max_length, type=int)
    parser.add_argument(
    ...
    print("argumentus")
    print(arguments.confidence_low)
    print(arguments.confidence_high)
    print("-------------------")
...
def visual_search_queue(vsm_args, vsm, image, target_object_name, current_patch, search_path, queue, smallest_size=224, confidence_high=0.5, target_cue_threshold=6.0, target_cue_threshold_decay=0.7, target_cue_threshold_minimum=3.0):
    ...
    while(not queue.empty()):
        ...
        success, search_path, all_valid_boxes = visual_search_queue(vsm_args, vsm, image, target_object_name, patch_chosen, search_path, queue, smallest_size=smallest_size, confidence_high=confidence_high, target_cue_threshold=target_cue_threshold, target_cue_threshold_decay=target_cue_threshold_decay, target_cue_threshold_minimum=target_cue_threshold_minimum)
        if success:
            return success, search_path, all_valid_boxes
    return False, search_path, None


def visual_search(vsm_args, vsm, image, target_object_name, target_bbox, smallest_size, confidence_high=0.5, confidence_low=0.3, target_cue_threshold=6.0, target_cue_threshold_decay=0.7, target_cue_threshold_minimum=3.0, visualize=False, save_path=None):
    confidence_high = vsm_args.confidence_high
    ...
    queue = PriorityQueue()
    search_successful, search_path, all_valid_boxes = visual_search_queue(vsm_args, vsm, image, target_object_name, init_patch, search_path, queue, smallest_size=smallest_size, confidence_high=confidence_high, target_cue_threshold=target_cue_threshold, target_cue_threshold_decay=target_cue_threshold_decay, target_cue_threshold_minimum=target_cue_threshold_minimum)
